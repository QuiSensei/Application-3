...
bench.load("./Models/Bench1/scene.gltf", (gltf) => {
  ...
  // Set position
  bench.position.set(0, 0, -6.5); // X, Y, Z coordinates
...
const hedge1 = new GLTFLoader();
hedge1.load("./Models/Hedge/scene.gltf", (gltf) => {
  const hedge = gltf.scene;

  hedge.position.set(3, 0, -8.5);

  hedge.scale.set(0.05, 0.03, 0.05);
...
const hedge2 = new GLTFLoader();
hedge2.load("./Models/Hedge/scene.gltf", (gltf) => {
  const hedge = gltf.scene;
...
function init() {
  ...
  dayAndNightLight = new THREE.DirectionalLight("#ffffff", 2); // Brighter directional light
  dayAndNightLight.position.set(-15, 5.5, 10); // Simulate sun high in the sky
...
function toggleDayNight() {
  if (currentMode === "day") {
    ...
    gsap.to(dayAndNightLight.position, { x: 0, y: 10, z: -25 });
    gsap.to(dayAndNightLight.color, { r: 0.1, g: 0.1, b: 0.1, duration: 2 }); // Black
    gsap.to(dayAndNightLight, { intensity: 5, duration: 2 }); // Dimmer light

    // Ambient light changes
    gsap.to(ambientLight.color, { r: 0.1, g: 0.1, b: 0.1, duration: 1.5 }); // Dim ambient light
    gsap.to(ambientLight, { intensity: 1.5, duration: 2 }); // Reduced intensity
    ...
    // dayAndNightLight.position.set(-15, 10, 10)
    gsap.to(dayAndNightLight.position, { x: -15, y: 5.5, z: 10 });
    gsap.to(dayAndNightLight.color, { r: 1.0, g: 1.0, b: 1.0, duration: 2 }); // Bright white
    gsap.to(dayAndNightLight, { intensity: 2, duration: 2 }); // Full brightness

    // Ambient light changes
    gsap.to(ambientLight.color, { r: 0.73, g: 0.84, b: 1.0, duration: 2 }); // Soft daylight color
    gsap.to(ambientLight, { intensity: 1, duration: 2 }); // Normal intensity
